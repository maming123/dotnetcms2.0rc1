...
namespace Foosun.SQLServerDAL
{
	public class Pagination : DbBase, IPagination
	{
		...
		protected string[] manage_label_style_3 = { "id", "id,styleID,StyleName,CreatTime,Content", "sys_LabelStyle where StyleName like cast(@Keyword as nvarchar) or styleID like cast(@Keyword as nvarchar) and  isRecyle=0 and SiteID=@SiteID", "order by Id desc" };
		...
		//protected string[] user_friend_friendmanage ={ "a.id", "a.id,a.HailFellow,a.FriendName,a.CreatTime,(select count(id) from " + Foosun.Config.UIConfig.dataRe + "user_Friend b where b.HailFellow=a.UserNum) as CNT", "User_FriendClass a where a.UserNum=@UserNum or a.gdfz='1'", "order by a.Id desc" };
		...
		protected string[] manage_label_syslabelbak = { "id", "id,LabelID,Label_Name,CreatTime", "sys_Label Where SiteID=@SiteID And isRecyle=0 And isBack=1", "order by Id desc" };
		...
		//投稿后台
		protected string[] manage_Contribution_Constrchicklist = { "id", "id,ConID,Title,creatTime,UserNum,Source,Tags,Author,ispass,Contrflg", "User_Constr where substring(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=1 and ispass=0 and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };

		protected string[] manage_Contribution_ConstrList = { "id", "id,ConID,Title,creatTime,Source,Tags,Contrflg,Author,ispass", "User_Constr where substring(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=0 and ispass=0 and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };
		...
		protected string[] manage_user_discussclass_3 = { "id", "id,DcID,Cname,Content", "User_DiscussClass where indexnumber='0' and Cname like cast(@titlem as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };
		...
		protected string[] manage_user_discuss_2 = { "id", "id,DisID,Cname,UserName,Creatime,Authoritymoney", "user_Discuss where Cname like cast(@titlem as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };
		...
		protected string[] manage_survey_setClass_2 = { "vid", "vid,ClassName,Description", "vote_Class where VID like cast(@VID as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by vid desc" };

		protected string[] manage_survey_setClass_3 = { "vid", "vid,ClassName,Description", "vote_Class where ClassName like cast(@ClassName as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by vid desc" };

		protected string[] manage_survey_setClass_4 = { "vid", "vid,ClassName,Description", "vote_Class where Description like cast(@Description as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by vid desc" };
		//调查选项
		protected string[] manage_survey_setItem_1 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where TID like cast(@TID as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_2 = { "IID", "IID,TID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_3 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemName like cast(@ItemName as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_4 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemValue like cast(@ItemValue as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_5 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where PicSrc like cast(@PicSrc as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_6 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where DisColor like cast(@DisColor as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_7 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where VoteCount like cast(@VoteCount as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setItem_8 = { "TID", "TID,IID,ItemName,ItemValue,ItemMode,PicSrc,DisColor,VoteCount,ItemDetail", "vote_Item where ItemDetail like cast(@ItemDetail as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };
		//多步投票管理
		...
		protected string[] manage_survey_setSteps_2 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where SID like cast(@SID as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by sid desc" };

		protected string[] manage_survey_setSteps_3 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where TIDS like cast(@TIDS as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by sid desc" };

		protected string[] manage_survey_setSteps_4 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where Steps like cast(@Steps as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by sid desc" };

		protected string[] manage_survey_setSteps_5 = { "SID", "SID,TIDS,Steps,TIDU", "vote_Steps where TIDU like cast(@TIDU as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by sid desc" };
		//投票主题
		...
		protected string[] manage_survey_setTitle_2 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where Title like cast(@Title as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setTitle_3 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where VID like cast(@VID as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setTitle_4 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where MaxNum like cast(@MaxNum as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setTitle_5 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where StartDate like cast(@StartDate as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setTitle_6 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where EndDate like cast(@EndDate as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };

		protected string[] manage_survey_setTitle_7 = { "TID", "TID,VID,Title,Type,MaxNum,DisMode,StartDate,EndDate,ItemMode", "vote_Title where ItemMode like cast(@ItemMode as nvarchar) and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by TID desc" };
		//自定义字段
		...
		public DataTable GetPage(string PageName, int PageIndex, int PageSize, out int RecordCount, out int PageCount, params SQLConditionInfo[] SqlCondition)
		{
			...
			if (!_dirName.Equals("manage"))
			{
				...
				{
					PageType = PageType.Substring(_dirName.Length);
