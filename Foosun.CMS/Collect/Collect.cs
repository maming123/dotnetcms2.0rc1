...
namespace Foosun.CMS.Collect
{
	...
	public class Collect {
		private Foosun.IDAL.ICollect dal;
		...
		public void Collecting(int folderid, int num, bool bnorepeat) {
			...
			if (bool.Parse(r["SaveRemotePic"].ToString())) {
				...
				PicSavePath = Common.ServerInfo.GetRootPath().TrimEnd('\\') + @"\" + rtpath + @"\RemoteFiles\" + dtpath;
				if (!Directory.Exists(PicSavePath))
					Directory.CreateDirectory(PicSavePath);
				PicSaveUrl = Common.Public.GetSiteDomain() + "/" + rtpath + "/RemoteFiles/" + dtpath;
				bSaveRemotePic = true;
				#endregion
			}
			#endregion 检查数据是否完整
			...
			if (NewsUrl.Length < num) {
				string prompt = "正在采集新闻,终止<a href=\"Collect_List.aspx\">返回</a>.成功:" + nSucceed * 100 / num + "% ";
				if (nRepeat > 0)
					prompt += "(其中重复:" + nRepeat * 100 / num + "%) ";
				prompt += "失败:" + nFailed * 100 / num + "%";
				HProgressBar.Roll(prompt, 100);
			}
		}
		/// <summary>
		/// 处理采集单条新闻
		...
		private int CollectPage(string Url, DataRow r, bool norepeat) {
			...
			pn.FigureTitle();
			if (string.IsNullOrEmpty(pn.Title)) {
				return 1;
			}
			if (norepeat) {
			...
			if (pn.Content != null && !pn.Content.Trim().Equals("") && !pn.Title.Trim().Equals("")) {
				...
				//处理顾虑字符串的作用（wxh 2008-7-18）                
				string Content = GetNewsContent(r["ID"].ToString(), pn.Content);
				if (bSaveRemotePic) {
				...
				if (Foosun.Config.UIConfig.enableAutoPage == "true") {
					ninf.Content = Common.Input.AutoSplitPage(ninf.Content, int.Parse(Foosun.Config.UIConfig.splitPageCount));
				}
				//获取内容中新闻图片的数量
				Regex regexImg = new Regex("<img", RegexOptions.IgnoreCase);
				MatchCollection matchesImg = regexImg.Matches(Content);
				ninf.ImagesCount = matchesImg.Count;
		...
		/// <summary>
		/// 将从网上获得的新闻内容进行字符串过滤（wxh 2008-7-18）
		/// </summary>
		/// <param name="siteID"></param>
		/// <param name="oldNewsContent"></param>
		/// <returns></returns>
		private string GetNewsContent(string siteID, string oldNewsContent) {
			string newsContent = "";
			DataTable dt = GetRuleApplyList();
			if (dt.Rows.Count > 0) {
				foreach (DataRow dr in dt.Rows) {
					if (dr["SiteID"].ToString() == siteID) {
						string oldRule = dr["OldContent"].ToString();
						string newRule = dr["ReContent"].ToString();
						Regex reg = null;
						if (oldRule.IndexOf("[变量]") == -1) {
							reg = null;
						}
						else if (oldRule.Replace("[变量]", null).Length > 0) {
							reg = new Regex(oldRule.Replace("[变量]", ".+?"), RegexOptions.Compiled | RegexOptions.IgnoreCase);//过滤正则表达式最小匹配
						}
						else {
							reg = new Regex(oldRule.Replace("[变量]", ".+"), RegexOptions.Compiled | RegexOptions.IgnoreCase);//过滤正则表达式最大匹配
							oldNewsContent = oldNewsContent.Replace("\n", null);
						}
						if (reg != null) {
							MatchCollection matches = reg.Matches(oldNewsContent);
							if (matches.Count > 0) {
								foreach (Match match in matches) {
									oldNewsContent = oldNewsContent.Replace(match.Value, newRule);
									newsContent = oldNewsContent;
								}
							}
							else {
								newsContent = oldNewsContent;
							...
							newsContent = oldNewsContent.Replace(oldRule, newRule);
						}
					}
					else {
						newsContent = oldNewsContent;
		...
		public void StorageNews(string id) {
			...
			try {
				...
				HTextProgressBar.EndProgress("采集新闻入库已完成。共成功:" + ns + "条,失败:" + nf + "条新闻(可能是入库栏目不存在)。" + s);
			}
			catch (Exception ex) {
