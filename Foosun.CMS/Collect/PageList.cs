...
namespace Foosun.CMS.Collect
{
    public class PageList : Page
    {
        ...
        public void FigureNewsUrls()
        {
            ...
            while (m.Success)
            {
                _NewsUrlList.Add(Utility.StickUrl(baseUrl, m.Groups["TARGET"].Value));
                m = m.NextMatch();
            }
        }
        ...
        public string[] SinglePagination(string profile, int total)
        {
            ...
            while (m.Success)
            {
                if (n >= total)
                    break;
                string otherurl = Utility.StickUrl(baseUrl, m.Groups["TARGET"].Value);
        ...
        public string[] IndexPagination(string profile, int min, int max, int total)
        {
            ...
            for (int i = min; i <= max; i++)
            {
                ...
                MatchCollection matchHref = regexHref.Matches(otherurl);
                foreach (Match m in matchHref)
                {
                    otherurl = m.Value;
                    break;
                }
                otherurl = otherurl.Replace("href='", "");
                try
                {
                    otherurl = Utility.StickUrl(baseUrl, otherurl);
                }
                catch
                {
                    otherurl = "";
                ...
                if (!otherurl.Trim().Equals(this._Url.Trim(), StringComparison.CurrentCultureIgnoreCase))
                {
                    PageList pglst = new PageList(otherurl, _Encode);
                    ArrayList arraylist = GetListUrl(pglst);
                    if (arraylist != null && arraylist.Count > 0)
