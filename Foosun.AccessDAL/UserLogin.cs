...
namespace Foosun.AccessDAL
{
    public class UserLogin : DbBase, IUserLogin
    {
        ...
        {
            OleDbConnection cn = new OleDbConnection(Foosun.Config.DBConfig.CmsConString);
            try
            {
                cn.Open();
                AdminDataInfo info;
        ...
        protected IDataReader GetUserGroupInfo(OleDbConnection cn, string groupNum)
        {
            ...
            {
                rd.Close();
                return (OleDbDataReader)DbHelper.ExecuteReader(cn, CommandType.Text, SQL_DEFUSERGROUP, null);
            }
            return rd;
        }
        ...
        protected void UpdateUserLogin(OleDbConnection cn, UserLoginSucceedInfo info)
        {
            try
            {
                ...
                Param[0] = new OleDbParameter("@LastIP", OleDbType.VarChar, 15);
                Param[0].Value = info.IP;
                Param[1] = new OleDbParameter("@UserGroupNumber", OleDbType.VarChar, 20);
                Param[1].Value = newGroup;
                Param[2] = new OleDbParameter("@UserNum", OleDbType.VarChar, 20);
