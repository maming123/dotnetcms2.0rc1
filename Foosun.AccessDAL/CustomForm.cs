...
namespace Foosun.AccessDAL
{
    public class CustomForm : DbBase, ICustomForm
    {
        void ICustomForm.Edit(CustomFormInfo info)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Sql += " [submit_time] date null,";
                    Sql += " [manage_name] varchar (50) null,";
                    Sql += " [manage_content] varchar (255) null ,";
                    Sql += " [manage_addtime] date null ,";
                    Sql += " [isshow] int default 0"; //默认不显示
                    Sql += ")";
        ...
        {
            string sql = "select isnotnull from " + Pre + "customform_item where formid=" + formid + " and fieldname='" + filedname + "'";
