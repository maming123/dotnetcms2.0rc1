namespace Foosun.Publish
{
    ...
    public class UltiPublish
    {
        ...
        private StringBuilder sbNews = new StringBuilder();
        ...
        public static Thread[] thss;
        private string userName = string.Empty;
        private int userPublishID = 0;

        public UltiPublish(bool isProgressBar)
        {
            this.fs_isProgressBar = isProgressBar;
            this.SiteRootPath = ServerInfo.GetRootPath();
            try
            {
                foreach (int num in _userPublishInfo.Keys)
                {
                    ShowMessage message = (ShowMessage) _userPublishInfo[num];
                    DateTime lastPublichTime = message.LastPublichTime;
                    if (DateTime.Now.Subtract(lastPublichTime).Minutes > 15)
                    {
                        _userPublishInfo.Remove(num);
                    }
                }
            }
            catch
        ...
        public void BeganToPublish()
        {
            ...
            if (this.fs_isPubNews)
            {
                this.ultiPublishNews();
            }
            if (SearchEngine.IsBaidu() == "1")
            {
                SearchEngine.RefreshBaidu();
            }
            if (!(((this.fs_isPubindex || this.fs_isPubNews) || (this.fs_isPubClass || this.fs_isPubSpecial)) || this.fs_isPubIsPage))
            {
                HProgressBar.Roll("发布主页完成。&nbsp; <a class=\"xa3\" href=\"../../index.html\" target=\"_blank\">浏览首页</a>", 100);
            }
        }
        ...
        public string GetPagelist(string Numstr, int i, string getFileName, string getFileEXName, string postResult_color, string postResult_css, int n, string ID, string ContentType, int isPop, string CurrentSytle, string OtherStyle)
        {
            ...
            switch (Numstr)
            {
                case "0":
                    obj2 = str;
                    str = string.Concat(new object[] { obj2, "<div", postResult_css, "><span>共", n, "页</span><span>第", i + 1, "页</span>" });
                    if (i == 0)
                    {
        ...
        public bool publishSingleClass(string classID, string datalib)
        {
            try
            {
                ...
                string savePath = HttpContext.Current.Server.MapPath(this.saveClassPath);
        ...
        {
            ShowMessage message = (ShowMessage) _userPublishInfo[this.userPublishID];
            DataRow[] rowArray = message.ThreadRt.Select("newsID='" + newsID + "'");
            ...
            try
            {
                ...
                if (row["isDelPoint"].ToString() == "0")
                {
                    temppath = row["templet"].ToString().Replace("/", @"\").ToLower().Replace("{@dirtemplet}", strgTemplet);
                    temppath = str3.Trim(new char[] { '\\' }) + temppath;
                    str = @"\" + row["SavePath1"].ToString().Trim(new char[] { '\\' }).Trim(new char[] { '/' }) + @"\" + row["SaveClassframe"].ToString().Trim(new char[] { '\\' }).Trim(new char[] { '/' }) + @"\" + row["SavePath"].ToString().Trim(new char[] { '\\' }).Trim(new char[] { '/' }) + @"\" + row["FileName"].ToString().Trim(new char[] { '\\' }).Trim(new char[] { '/' }) + row["FileEXName"].ToString().Trim(new char[] { '\\' }).Trim(new char[] { '/' });
                    Template template = new Template(temppath, TempType.News);
                    template.NewsID = newsID;
                    template.ClassID = classID;
                    template.GetHTML();
                    template.IsContent = row["CommTF"].ToString() == "1";
                    Public.savePublicLogFiles("发布开始时间--------------------------", DateTime.Now.ToString("HH:mm:ss.fffffff"), "发布开始时间--------------------");
                    template.ReplaceLabels();
                    Public.savePublicLogFiles("发布结束时间--------------------------", DateTime.Now.ToString("HH:mm:ss.fffffff"), "发布结束时间--------------------");
                    ...
                    if (template.MyTempType == TempType.News)
                    {
                        ...
                        if (index > -1)
                        {
                            ...
                            for (int i = 0; i < length; i++)
                            {
                                ...
                                General.WriteHtml(General.ReplaceResultPage(row["NewsID"].ToString(), str12.Replace("FS:END->", "").Replace("<!-FS:STAR=", ""), fileName, eXName, length, i + 1, 0), filePath);
                            }
                        }
                        else
                        {
        ...
        public bool publishSingleSpecial(string specialID)
        {
            try
            {
                Template template2;
                this.TempletPath = this.SiteRootPath.Trim(new char[] { '\\' }) + this.TempletPath;
                string savePath = HttpContext.Current.Server.MapPath(this.saveSpecialPath);
                bool existFlag = false;
                if (this.templateList.Count != 0)
                {
                    foreach (Template template in this.templateList)
        ...
        private void thread()
        {
            ...
            try
            {
                string str;
                DataRow row = null;
                for (int i = 0; i < message.ThreadRt.Rows.Count; i++)
                {
                    this.SetNewsCatch(i, message.NewsIdNameList);
                    row = message.ThreadRt.Rows[i];
                    message.ThreadFlag = true;
                    this.TempletPath = row["templet"].ToString();
                    this.TempletPath = this.TempletPath.Replace("/", @"\");
                    this.TempletPath = this.TempletPath.ToLower().Replace("{@dirtemplet}", this.strTempletDir);
                    this.setSaveNewsPath(row["classID"].ToString(), row["SavePath"].ToString(), row["FileName"].ToString(), row["FileEXName"].ToString());
                    bool flag = this.publishSingleNews(row["newsID"].ToString(), row["datalib"].ToString(), row["classID"].ToString());
                    message.ThisPublisCount++;
                    if (flag)
                    {
                        message.Success++;
                    }
                    ...
                    message.BarNum = (message.ThisPublisCount * 100) / this.nNewsCount;
                    message.MaxPublishNumber = this.nNewsCount;
                    _userPublishInfo[this.userPublishID] = message;
                    message.PubName = "新闻";
                    str = string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "新闻", this.nNewsCount.ToString(), message.ThisPublisCount, message.Success, message.Error, "发布新闻成功。&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + message.Indexname + "\" target=\"_blank\">浏览首页</a> &nbsp;&nbsp;&nbsp; " });
                    this.showMSGNotThread(str, message.BarNum);
                }
                if (!message.ThreadFlag)
                {
                    str = string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "新闻", this.nNewsCount.ToString(), message.ThisPublisCount, message.Success, message.Error, "发布新闻成功。&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + message.Indexname + "\" target=\"_blank\">浏览首页</a> &nbsp;&nbsp;&nbsp; " });
                ...
                {
                    for (int j = 0; j < this.failedList.Count; j++)
                    {
                        Public.savePublicLogFiles("□□□发布新闻", "【ID】:" + this.failedList[j].Split(new char[] { '$' })[0] + "\r\n【错误描述：】\r\n" + this.failedList[j].Split(new char[] { '$' })[1], "");
                    }
                    this.failedList.Clear();
                }
                if (this.succeedList.Count != 0)
                {
                    this.updateNewsIsHtml(DBConfig.TableNamePrefix + "news", "isHtml", "newsID");
                }
                _userPublishInfo[this.userPublishID] = message;
            }
            catch
            ...
            finally
            {
                ...
                _userPublishInfo.Remove(this.userPublishID);
                this.clearPublicshCatch();
            }
        }
        ...
        public void ultiPublishClass()
        {
            using (IDataReader reader = this.GetAllClass())
            {
                ...
                StringBuilder builder = new StringBuilder();
                builder.Append("<script type=\"text/javascript\" src=\"/Scripts/public.js\" charset=\"gb2312\"></script>");
                builder.Append("<script type=\"text/javascript\" src=\"/Scripts/jquery.js\"></script>");
                ...
                if (this.nClassCount > 0)
                {
                    while (reader.Read())
                    {
                        ...
                        bool flag2 = this.publishSingleClass(classID, reader["Datalib"].ToString());
                        ...
                        HProgressBar.Roll(string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "栏目", this.nClassCount.ToString(), num.ToString(), num2.ToString(), num3.ToString(), "发布栏目成功&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a>&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + str + "\" target=\"_blank\">浏览首页</a>" }), (num * 100) / this.nClassCount);
                    }
                }
                else
                {
                ...
                if (!flag)
                {
                    string msg = string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "栏目", this.nClassCount.ToString(), num.ToString(), num2.ToString(), num3.ToString(), "发布栏目成功&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a>&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + str + "\" target=\"_blank\">浏览首页</a>" });
                    if (this.nClassCount > 0)
                    {
                        HProgressBar.Roll(msg, (num * 100) / this.nClassCount);
                    }
                    else
                    {
                        HProgressBar.Roll("没有栏目", 0);
        ...
        private void ultiPublishIndex()
        {
            ...
            HProgressBar.Roll("发布主页成功。&nbsp; <a class=\"xa3\" href=\"../../" + str + "\" target=\"_blank\">浏览首页</a>", 100);
        }
            ...
            if (this.fs_strClassIsPageParam != null)
            {
                ...
                if (this.nClassCount > 0)
                {
                    while (reader.Read())
                    {
                        ...
                        HProgressBar.Roll(string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "单页", this.nClassCount.ToString(), num2.ToString(), num3.ToString(), num4.ToString(), "发布单页成功&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a>&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + str2 + "\" target=\"_blank\">浏览首页</a>" }), (num2 * 100) / this.nClassCount);
                    }
                }
                else
                {
                ...
                if (!flag)
                {
                    string msg = string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", new object[] { "单页", this.nClassCount.ToString(), num2.ToString(), num3.ToString(), num4.ToString(), "发布单页成功&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a>&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + str2 + "\" target=\"_blank\">浏览首页</a>" });
                    if (this.nClassCount > 0)
                    {
                        HProgressBar.Roll(msg, (num2 * 100) / this.nClassCount);
                    }
                    else
                    {
                        HProgressBar.Roll("没有单页", 0);
        ...
        private void ultiPublishNews()
        {
            ...
            else
            {
                ...
                builder2.Append("<script type=\"text/javascript\" src=\"/Scripts/public.js\" charset=\"gb2312\"></script>");
                builder2.Append("<script type=\"text/javascript\" src=\"/Scripts/jquery.js\"></script>");
                builder2.Append("<script type=\"text/javascript\" charset=\"utf-8\">");
                builder2.Append("var userPublishID=" + this.userPublishID + ";");
                builder2.Append("showMessageRequest('" + UIConfig.dirMana + "');");
                builder2.Append("</script>");
                HttpContext.Current.Response.Write(builder2.ToString());
                HttpContext.Current.Response.Flush();
                ths = new Thread(new ThreadStart(this.thread));
                ths.Priority = ThreadPriority.Lowest;
                ths.Name = ServerInfo.ServerPort;
                ths.Start();
            }
        }
        ...
        public void ultiPublishSpecial()
        {
            ...
            if (this.nSpecialCount > 0)
            {
                while (allSpecials.Read())
                {
                    ...
                    if (this.publishSingleSpecial(allSpecials["specialID"].ToString()))
                    {
                        num++;
                        num2++;
                    }
                    ...
                    objArray[0] = "专题";
                    objArray[1] = this.nSpecialCount.ToString();
                    int num5 = (int) num;
                    objArray[2] = num5.ToString();
                    objArray[3] = num2.ToString();
                    objArray[4] = num3.ToString();
                    objArray[5] = "发布栏目成功。&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a> &nbsp;&nbsp;&nbsp;<a class=\"xa3\" href=\"../../" + str + "\" target=\"_blank\">浏览首页</a>";
                    HProgressBar.Roll(string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", objArray), ((int) (num / ((float) this.nSpecialCount))) * 100);
                }
            }
            else
            {
            ...
            if (!flag)
            {
                objArray = new object[] { "专题", this.nSpecialCount.ToString(), ((int) num).ToString(), num2.ToString(), num3.ToString(), "发布栏目成功。&nbsp;&nbsp;<a class=\"xa3\" href=\"javascript:publishClose()\">关 闭</a> &nbsp;&nbsp;&nbsp; <a class=\"xa3\" href=\"../../" + str + "\" target=\"_blank\">浏览首页</a>" };
                HProgressBar.Roll(string.Format("正在发布{0}，共需要发布{1}条，正在发布第{2}条，成功{3}条， <a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败{4}条</span></a>{5}", objArray), ((int) (num / ((float) this.nSpecialCount))) * 100);
            }
            if (this.templateList.Count != 0)
            {
